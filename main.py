...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 أهلاً بك! أرسل رابط الفيديو للتحميل.")
...
async def run_down(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    mode, url = query.data.split("|")
    msg = await query.edit_message_text("⏳ جاري المعالجة...")

    opts = {'format': 'best' if mode == 'v' else 'bestaudio/best', 'outtmpl': 'file.%(ext)s'}
    if mode == 'a':
