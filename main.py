...
# --- قاعدة البيانات ---
def init_db():
    conn = sqlite3.connect('pro_v5.db')
    c = conn.cursor()
...
# --- إعدادات التحميل الشاملة (يوتيوب، فيس، إنستا، تيك توك) ---
def get_ydl_opts(mode, url):
    # كشف المنصة لتطبيق الإعدادات المناسبة
    ...
    if is_tiktok:
        # إعدادات خاصة لتيك توك لتجاوز القيود
        opts['extractor_args'] = {'tiktok': {'web_proxy': True}}
    elif not is_youtube:
        opts['extractor_args'] = {
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit()
    conn.close()
    await update.message.reply_text("✅ تم تحديث البوت! يدعم الآن:\n(يوتيوب، فيسبوك، إنستا، تيك توك) 🚀")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text and "http" in text:
        # حفظ الرابط لتجنب خطأ Button_data_invalid
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "broadcast":
        ...
        await query.message.reply_text("📥 أرسل المحتوى للإذاعة.")
        return
    ...
    try:
        ...
        await query.message.reply_text("⚠️ فشل التحميل. قد يكون الفيديو خاصاً أو محذوفاً.")
...
if __name__ == '__main__':
    ...
    # حل مشكلة الـ Conflict وتكرار العمليات
